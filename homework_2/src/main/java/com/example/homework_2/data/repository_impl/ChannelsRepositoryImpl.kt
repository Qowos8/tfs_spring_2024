...
import com.example.homework_2.data.network.model.channels.stream.StreamCreateItem
...
internal class ChannelsRepositoryImpl @Inject constructor(
    ...
    override suspend fun searchStream(query: String, initial: List<StreamItem>): List<StreamItem> {
        return FilterStreamsByName.filterItemsByName(initial, query)
    }

    override suspend fun createStream(name: String, description: String) {
        api.createChannel(
            name = Json.encodeToString(listOf(StreamCreateItem(name, description)))
        )
    }
